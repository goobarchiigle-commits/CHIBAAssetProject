...
# Single Source of Truth / 最終更新: 2026-03-30（スリーステップ完了 / RSR42統一 / エグジット感度テスト）
...
**次の優先タスク（更新）**:
- [x] ~~min_rsr 感度テスト~~ **完了・逆効果確定**
- [x] ~~RSR42ユニバース統一（Step 1）~~ **完了**
- [x] ~~OOS再検証（Step 2）~~ **完了: -5.7%、2025特殊相場確定**
- [x] ~~エグジット感度テスト（Step 3）~~ **完了: turtle_exitはOOS改善に効かない**
- [ ] **exit55d 採用判断**: IS Sharpe 1.186→1.335（+12.6%）、OOS微悪化（-5.7%→-6.0%）を許容できるか
- [ ] **最低保有日数（min_hold=5）テスト**: RSRモメンタムエグジットのノイズシグナル除去
- [ ] **rsr75_pos5 採用判断**: IS MaxDD -23.4% を許容できるか（OOS最良: -4.1%）
- [ ] breadth修正: price>MA200 + TOPIX150MA の実装
