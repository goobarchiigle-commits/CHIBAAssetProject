...
from src.paths import UNIVERSE_DIR
...
def run_backtest(
    ...
    # ── SNAPSHOT ARCHAEOLOGY (2026-06-25): composite_alpha_bt.run_scenario() と
    # 同一定義のSortino/ProfitFactor/AvgHoldDays/TradesPerYearを追加し、指標統一比較を可能にする ──
    ...
    hold_days = [t["exit_idx"] - t["entry_idx"] for t in sell_trades]
    ...
    return {
        ...
        "n_trades_yr":   len(sell_trades) / max(years, 0.01),
        "avg_hold_days": avg_hold_days,
        "win_rate":      win_rate,
        "equity_curve":  equity_series,
        "exposure_curve": pd.Series(exposure_list, index=eq_dates),
        "trades":        trades,
    }
