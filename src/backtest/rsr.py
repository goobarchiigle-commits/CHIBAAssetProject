...
def calc_composite_alpha_matrix(
    ...
    実装:
      - sliding_window_view を使った view ベース演算
      - Series / dict / DataFrame 中間生成を減らす
      - float32 出力
    """
    if not universe_prices:
        return pd.DataFrame(dtype=np.float32)

    prices_df = pd.DataFrame(universe_prices)
    n_rows, n_cols = prices_df.shape
    out = np.full((n_rows, n_cols), np.nan, dtype=np.float32)
    if n_rows < window:
        return pd.DataFrame(out, index=prices_df.index, columns=prices_df.columns, dtype=np.float32)

    arr = prices_df.to_numpy(dtype=np.float32, copy=True)
    x_raw = np.arange(window, dtype=np.float32)
    x_std = (x_raw - x_raw.mean()) / (x_raw.std() + np.float32(1e-8))
    x_std = x_std.astype(np.float32, copy=False)

    for col_idx in range(n_cols):
        log_p = np.log(np.maximum(arr[:, col_idx], np.float32(1e-8))).astype(np.float32, copy=False)
        windows = sliding_window_view(log_p, window_shape=window)
        slopes = (windows @ x_std) / np.float32(window)
        y_means = windows.mean(axis=1)
        y_preds = slopes[:, None] * x_std[None, :] + y_means[:, None]
        ss_res = ((windows - y_preds) ** 2).sum(axis=1)
        ss_tot = ((windows - y_means[:, None]) ** 2).sum(axis=1)
        r2 = np.where(ss_tot > np.float32(1e-8), 1.0 - ss_res / ss_tot, 0.0)
        out[window - 1:, col_idx] = (slopes * np.maximum(r2, 0.0)).astype(np.float32, copy=False)
