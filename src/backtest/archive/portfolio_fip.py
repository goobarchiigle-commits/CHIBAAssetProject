"""
backtest/portfolio_fip.py
FIP（Frog in the Pan）フィルター効果検証

【仮説】
  Gray & Vogel「Quantitative Momentum」の知見:
  「急騰ではなくコツコツ上昇した銘柄だけを選ぶ」と
  MaxDDが削減されCalmar比が改善する。

  G宇宙(27銘柄/max3)がCAGR+11.13%/MaxDD-6.62%/Calmar=1.681。
  FIPフィルターでMaxDDを-5.5%以下に抑えれば Calmar 2.0 達成。

【比較シナリオ】
  G_base  : 27銘柄 / max3 / Vol調整 / フィルターなし (前回Calmar=1.681)
  G_fip52 : 27銘柄 / max3 / Vol調整 / FIP≥52%
  G_fip55 : 27銘柄 / max3 / Vol調整 / FIP≥55%
  GC_fip  : 27銘柄 / max3 / Vol調整 / FIP≥52% + Clenowスコアソート

【実行方法】
  python -m backtest.portfolio_fip
"""
...
from backtest.rsr                     import calc_universe_rsr, calc_universe_clenow, calc_universe_fip
...
# (label, min_sharpe, max_dd_filter, max_pos, fip_threshold, use_clenow)
SCENARIOS = [
    ("G_base:   27銘柄/max3/FIPなし",     0.3, 0.30, 3, None,  False),
    ("G_fip52:  27銘柄/max3/FIP≥52%",    0.3, 0.30, 3, 0.52,  False),
    ("G_fip55:  27銘柄/max3/FIP≥55%",    0.3, 0.30, 3, 0.55,  False),
    ("GC_fip:   27銘柄/max3/FIP+Clenow", 0.3, 0.30, 3, 0.52,  True),
    ("F_fip52:  21銘柄/max2/FIP≥52%",    0.4, 0.25, 2, 0.52,  False),
]
...
def main() -> None:
    print("=" * 72)
    print("  FIP（Frog in the Pan）フィルター効果検証")
    print("  Gray & Vogel「Quantitative Momentum」— コツコツ上昇銘柄優先")
    ...
    print(f"  FIP比率計算中（window={FIP_WINDOW}日）...")
    fip_universe = calc_universe_fip(universe_prices, window=FIP_WINDOW)
    print(f"  FIP比率計算完了: {fip_universe.shape[1]} 銘柄")

    # FIPサマリー
    latest_fip = fip_universe.iloc[-1].sort_values(ascending=False)
    print(f"\n  FIP比率上位10銘柄（{fip_universe.index[-1].date()}時点）:")
    for sym, fip in latest_fip.head(10).items():
        sector = broad_tickers.get(sym, "")
        flag = "✓ コツコツ型" if fip >= 0.52 else "  急騰型"
        print(f"    {sym} ({sector}): {fip:.3f} {flag}")
    ...
    for label, min_shr, max_dd_filter, max_pos, fip_thr, use_clenow in SCENARIOS:
        ...
        fip_df    = fip_universe[[c for c in fip_universe.columns if c in tier_syms]] if fip_thr else None
        clenow_df = clenow_universe[[c for c in clenow_universe.columns if c in tier_syms]] if use_clenow else None
        ...
        fip_str = f"FIP≥{fip_thr:.0%}" if fip_thr else "FIPなし"
        clw_str = "+Clenow" if use_clenow else ""
        print(f"  ▶ {label} ({len(universe_t)}銘柄: FJ={fj_n} MR={mr_n} [{fip_str}{clw_str}])...")
        ...
        engine = PortfolioEngine(
            ...
            fip_scores        = fip_df,
            fip_min_ratio     = fip_thr if fip_thr else 0.0,
        )
        results[label] = engine.run()

    # ---- 比較テーブル ----
    print("\n\n" + "=" * 85)
    print("  FIPフィルター効果検証 — 比較結果")
    print("=" * 85)
    ...
    png_path = "data/portfolio_fip.png"

    fig, axes = plt.subplots(3, 1, figsize=(15, 13), sharex=True)
    fig.suptitle(
        "FIPフィルター（Frog in the Pan）効果検証（2018–2024）\n"
        "Gray&Vogel「Quantitative Momentum」— コツコツ上昇銘柄優先戦略",
        fontsize=12,
    )
    cmap   = plt.cm.tab10
