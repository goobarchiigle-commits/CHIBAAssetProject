...
MAX_NEW_PER_DAY   = 2
MIN_SEPA          = 6
MIN_RSR           = 75.0     # entry 閾値（変更禁止）
MIN_RSR_EXIT      = 70.0     # exit 閾値（WF 5-fold 検証済 2026-06-05）
MAX_DD_LIMIT      = 0.15
REENTRY_COOLDOWN  = 5
LOT               = 100
...
def run_backtest(
    ...
    for i, date in enumerate(common_dates):
        ...
        for sym in active_syms:
            ...
            # RSR 低下エグジット（exit 閾値 = rsr_exit、entry 閾値 min_rsr=75 とは別）
            if is_holding and rsr_val < MIN_RSR_EXIT:
