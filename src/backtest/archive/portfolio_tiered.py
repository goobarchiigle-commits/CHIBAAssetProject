"""
backtest/portfolio_tiered.py
段階的ユニバース拡張バックテスト

【比較シナリオ】
  C. 14銘柄  Phase1基準  (Sharpe>0.5, DD<20%) + Vol調整  [前回ベスト]
  F. 21銘柄  緩和基準A   (Sharpe>0.4, DD<25%) + 均等配分
  G. 27銘柄  緩和基準B   (Sharpe>0.3, DD<30%) + 均等配分
  H. 35銘柄  緩和基準C   (Sharpe>0.2, DD<35%) + 均等配分

目的: 銘柄数が増えるにつれてCAGR・Sharpe・DDがどう変化するかを測定する。
     ナラン理論「アルファの質が高い銘柄だけを増やす」の効果を検証。

【実行方法】
  python -m backtest.portfolio_tiered
"""
...
# 段階的閾値設定
TIERS = [
    ("C: 14銘柄/Phase1",  0.5, 0.20, True),   # (label, min_sharpe, max_dd, use_vol)
    ("F: 21銘柄/緩和A",   0.4, 0.25, False),
    ("G: 27銘柄/緩和B",   0.3, 0.30, False),
    ("H: 35銘柄/緩和C",   0.2, 0.35, False),
]
...
def main() -> None:
    print("=" * 72)
    print("  段階的ユニバース拡張バックテスト")
    print("  C: 14銘柄  F: 21銘柄  G: 27銘柄  H: 35銘柄")
    ...
    # ---- 2. 全対象銘柄（35銘柄）のデータ一括取得 ----
    broadest = df_sel[(df_sel["sharpe"] > 0.2) & (df_sel["maxdd"].abs() < 0.35)]
    ...
    for label, min_shr, max_dd, use_vol in TIERS:
        # 対象銘柄を抽出
        mask = (df_sel["sharpe"] > min_shr) & (df_sel["maxdd"].abs() < max_dd)
        ...
        for sym, info in universe_t.items():
            ...
            label_ = sym_to_strategy.get(sym, "フジコ法")
            strat_dict[sym] = make_strategy(sym, info["sector"], label_, rsr_s)
        ...
        print(f"  ▶ {label} ({len(universe_t)}銘柄: フジコ={fj_n} 平均回帰={mr_n}"
              f" {'Vol調整' if use_vol else '均等配分'})...")
    ...
    for lbl in results:
        header += f" {lbl:>20}"
    print(header)
    print("  " + "-" * (18 + 21 * len(results)))

    metrics = [
        ("年率CAGR",        lambda r: f"{r.cagr*100:>+7.2f}%"),
        ("総リターン(7年)", lambda r: f"{r.total_return*100:>+7.2f}%"),
    ...
    base_lbl = list(results.keys())[0]
    print(f"\n  === 改善幅（{base_lbl} 基準） ===")
    for lbl, res in list(results.items())[1:]:
        print(
            f"  {lbl}: "
            f"CAGR {(res.cagr-base.cagr)*100:>+6.2f}%pt  "
            f"Sharpe {res.sharpe_ratio-base.sharpe_ratio:>+6.3f}  "
            f"MaxDD {(res.max_drawdown-base.max_drawdown)*100:>+5.2f}%pt  "
            f"平均保有 {res.n_positions.mean()-base.n_positions.mean():>+5.2f}銘柄"
        )
    ...
    png_path = "data/portfolio_tiered.png"

    fig, axes = plt.subplots(3, 1, figsize=(15, 13), sharex=True)
    fig.suptitle(
        "段階的ユニバース拡張バックテスト（2018–2024）\n"
        "C:14銘柄/Phase1  F:21銘柄  G:27銘柄  H:35銘柄",
        fontsize=12,
    )
    cmap   = plt.cm.tab10
