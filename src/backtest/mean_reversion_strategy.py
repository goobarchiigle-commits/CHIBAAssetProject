...
def _calc_rsi_series(close_arr: np.ndarray, period: int = 5) -> np.ndarray:
    close_arr = np.asarray(close_arr, dtype=np.float32)
    n = close_arr.shape[0]
    ...
    gains = np.maximum(delta, 0.0).astype(np.float32, copy=False)
    losses = np.maximum(-delta, 0.0).astype(np.float32, copy=False)

    avg_gain = np.mean(gains[:period], dtype=np.float32)
    avg_loss = np.mean(losses[:period], dtype=np.float32)

    if avg_loss == 0:
        rsi[period] = np.float32(100.0)
    else:
        rs = avg_gain / avg_loss
        rsi[period] = np.float32(100.0 - (100.0 / (1.0 + rs)))

    alpha_prev = np.float32(period - 1)
    alpha_den = np.float32(period)
    for i in range(period, len(gains)):
        avg_gain = (avg_gain * alpha_prev + gains[i]) / alpha_den
        avg_loss = (avg_loss * alpha_prev + losses[i]) / alpha_den
        if avg_loss == 0:
            rsi[i + 1] = np.float32(100.0)
        else:
            rs = avg_gain / avg_loss
            rsi[i + 1] = np.float32(100.0 - (100.0 / (1.0 + rs)))
    return rsi
...
class MeanReversionStrategy(BaseStrategy):
    ...
    def precompute_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        ma200_arr = _rolling_mean_array(close_arr, self.ma_long)
        ma50_arr = _rolling_mean_array(close_arr, 50)
        rsi_arr = _calc_rsi_series(close_arr, self.rsi_period)

        in_position = False
        entry_price = np.float32(0.0)
        entry_bar = -1
        knife_mult = np.float32(1.0 - self.knife_threshold)
        ...
        for i in range(1, n):
            ...
            if np.isnan(ma200_i) or np.isnan(ma50_i) or np.isnan(rsi):
                continue

            if in_position:
                hold_days = i - entry_bar
                stop_price = entry_price * np.float32(1.0 - self.stop_loss_pct)
                if rsi >= self.rsi_exit or current_price <= stop_price or hold_days >= self.max_hold_days:
                    in_position = False
                    signals_arr[i] = -1
                continue
            ...
            if rsi >= self.rsi_entry:
                continue
            if current_price <= ma200_i:
                continue
            if current_price < ma50_i * knife_mult:
                continue
