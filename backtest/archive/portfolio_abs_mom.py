"""
backtest/portfolio_abs_mom.py
絶対モメンタムフィルター効果検証

【仮説】
  Antonacci「Dual Momentum」+ Clenow「インデックスフィルター」の知見:
  「TOPIX が下降トレンドのとき新規BUYを禁止」するだけでMaxDDが激減し
  Calmar比が2.0を突破できる。

【比較シナリオ】
  F_base : 21銘柄 / max2 / Vol調整 / フィルターなし （現在ベスト Calmar=1.825）
  F_12m  : 21銘柄 / max2 / Vol調整 / TOPIX 12ヶ月リターン>0でBUY許可
  F_200m : 21銘柄 / max2 / Vol調整 / TOPIX>200日MAでBUY許可（Clenow式）
  G_12m  : 27銘柄 / max3 / Vol調整 / TOPIX 12ヶ月リターン>0でBUY許可

【実行方法】
  python -m backtest.portfolio_abs_mom
"""
...
START   = "2017-01-01"   # TOPIXの12ヶ月リターン計算用に1年前からダウンロード
END     = "2024-12-31"
BT_START = "2018-01-01"  # バックテスト実際の開始日
CAPITAL = 2_000_000
...
# シナリオ: (label, min_sharpe, max_dd_filter, max_pos, benchmark_mode)
# benchmark_mode: None=フィルターなし, "12m"=12ヶ月リターン, "200ma"=200日MA
SCENARIOS = [
    ("F_base: フィルターなし",     0.4, 0.25, 2, None),
    ("F_12m:  TOPIX12ヶ月",        0.4, 0.25, 2, "12m"),
    ("F_200ma: TOPIX200日MA",      0.4, 0.25, 2, "200ma"),
    ("G_12m:  27銘柄+TOPIX12ヶ月", 0.3, 0.30, 3, "12m"),
]
...
def main() -> None:
    print("=" * 72)
    print("  絶対モメンタムフィルター効果検証")
    print("  Antonacci「Dual Momentum」+ Clenow「インデックスフィルター」")
    print(f"  期間: {BT_START} 〜 {END}  /  初期資本: ¥{CAPITAL:,}")
    print("=" * 72)

    # ---- 1. TOPIXダウンロード ----
    print("\n[1/4] TOPIX データ取得中...")
    # ^N225(日経平均) または 1306.T(TOPIX ETF) を使用
    topix_raw = yf.download("1306.T", start=START, end=END,
                            auto_adjust=True, progress=False)
    if topix_raw.empty or "Close" not in topix_raw.columns:
        # fallback: 日経平均
        topix_raw = yf.download("^N225", start=START, end=END,
                                auto_adjust=True, progress=False)
    topix = topix_raw["Close"].squeeze()
    if isinstance(topix.index, pd.MultiIndex):
        topix = topix.droplevel(1)
    topix.name = "TOPIX"
    print(f"  TOPIX取得: {len(topix)}日  ({topix.index[0].date()} 〜 {topix.index[-1].date()})")
    ...
    print(f"\n[2/4] データ取得中（{len(broad_tickers)}銘柄）...")
    universe_all = download_universe(broad_tickers, start=START, end=END)
    print(f"  取得完了: {len(universe_all)} 銘柄")

    # バックテスト開始日以降に絞る（TOPIXは2017からのまま残す）
    bt_start_ts = pd.Timestamp(BT_START)
    for sym in universe_all:
        df = universe_all[sym]["df"]
        universe_all[sym]["df"] = df[df.index >= bt_start_ts]

    # ---- 3. RSR計算 ----
    print("\n[3/4] RSR 計算中...")
    # RSR計算は2017年からの全データで（精度向上のため）
    universe_full = download_universe(broad_tickers, start=START, end=END)
    ...
    for label, min_shr, max_dd_filter, max_pos, bench_mode in SCENARIOS:
        ...
        # ベンチマーク設定
        benchmark  = topix if bench_mode is not None else None
        use_ma     = (bench_mode == "200ma")
        ...
        filter_str = {"12m": "TOPIX12ヶ月", "200ma": "TOPIX200MA", None: "フィルターなし"}[bench_mode]
        print(f"  ▶ {label} ({len(universe_t)}銘柄: FJ={fj_n} MR={mr_n} [{filter_str}])...")
    ...
    metrics = [
        ...
        ("市場上昇期間",    lambda r: f"{r.regime_ok.mean()*100:>8.1f}%"),
    ...
    png_path = "data/portfolio_abs_mom.png"

    fig, axes = plt.subplots(4, 1, figsize=(15, 16), sharex=True)
    fig.suptitle(
        "絶対モメンタムフィルター効果検証（2018–2024）\n"
        "Antonacci「Dual Momentum」+ Clenow「TOPIX200日MAフィルター」",
        fontsize=12,
    )
    cmap   = plt.cm.tab10
    ...
    # TOPIX と市場レジーム
    ax = axes[3]
    topix_bt = topix[topix.index >= pd.Timestamp(BT_START)]
    ax.plot(topix_bt.index, topix_bt.values, color="steelblue",
            linewidth=1.2, label="TOPIX ETF (1306.T)")
    # 200日MAを描画
    ma200 = topix.rolling(200).mean()
    ma200_bt = ma200[ma200.index >= pd.Timestamp(BT_START)]
    ax.plot(ma200_bt.index, ma200_bt.values, color="orange",
            linewidth=1.0, linestyle="--", label="200日MA")
    ax.set_ylabel("TOPIX ETF (円)")
    ax.legend(fontsize=8)
